<template>
  <q-layout-header>
    <q-toolbar
      ...
      <q-tabs color="white">
        <q-route-tab slot="title" :color="color" icon="home" :to="{name: 'about'}" replace hide="icon" label="Главная" ></q-route-tab>
        <q-route-tab  v-if="true || isAdmin || isOperator" slot="title" :color="color" icon="edit" :to="{name: 'editor-dictionary'}" replace hide="icon" label="Редактор" ></q-route-tab>
